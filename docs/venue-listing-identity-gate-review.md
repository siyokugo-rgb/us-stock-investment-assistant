...
| Gate | Verdict |
| --- | --- |
| ProviderSymbolBindingEvidence | **PARTIAL**（symbol→FIGI candidate まで。Security resolved ではない） |
| OpenFIGI listing external identity（venue-level FIGI） | **PARTIAL**（公式粒度の listing/venue-level **external identity 候補**になり得る。MIC が無くても FIGI 自体は無効化しない） |
| Standardized venue identity（ISO MIC） | **PARTIAL / MISSING**（現行未保持；`exchCode≠MIC`） |
| Price↔listing join（AV TIME_SERIES_DAILY） | **FAIL〜NO-GO**（**AV Price venue semantics 未確定**が独立 High blocker） |
...
→ Equity の response `figi` は OpenFIGI 上の **listing / trading-venue 粒度の外部 identity 候補**であり、内部 `SecurityId` ではない。  
→ unique venue-level FIGI は **OpenFIGI listing external identity candidate として成立し得る**（MIC が無いからといって FIGI の存在自体を無効にはしない）。  
→ ただし **「SecurityId resolved」や「AV PRICE と listing-level join 済み」には自動昇格しない**（validity / class / **Price venue semantics** / standardized MIC は別レイヤ）。

### 5.1.1 FIGI と MIC の責務分離（必須）

| Concept | Role |
| --- | --- |
| venue-level FIGI | OpenFIGI / FIGI 標準上の **listing/venue-level external identity** |
| Composite FIGI | 同一 country/market 内の venue-level FIGI **集約** |
| Share Class FIGI | 複数 country composite を跨ぐ **share-class 集約** |
| ISO MIC | **ISO 10383 標準 venue code**；cross-provider venue join / 標準 venue evidence |
| OpenFIGI `exchCode` | OpenFIGI 独自 filter/code（MIC ではない） |

MIC は standardized venue identification に必要になり得るが、**FIGI 候補の成立条件を置換しない**。  
**禁止:** 「MIC が無い ⇒ unique venue-level FIGI は listing identity として無意味」という過大主張。  
**禁止:** 「venue-level FIGI がある ⇒ AV PRICE と listing join 可」。
...
→ Standardized venue identity のコード体系は ISO MIC。  
→ 必要なのは **実際の listing / trading venue に適用される explicit MIC evidence**（推測禁止）。  
→ **Operating MIC を一律 MUST と断定しない。** Segment MIC が当該 listing に適用される場合は Segment MIC と corresponding Operating MIC の関係を ISO 10383 evidence に従って保持する。  
→ MIC は FIGI を補強する標準 venue evidence であり、**FIGI の listing external identity 候補を取り消すものではない**。

### 5.6 Provider symbol / Alpha Vantage Price venue semantics（独立 blocker）

- PRICE archive の request provenance のみ（SecurityId / FIGI / MIC / currency ではない）
- TIME_SERIES_DAILY は raw daily series（本 repo 契約上 currency unresolved）

**現行契約で未確定な最大ギャップ（High blocker）:**

`symbol=IBM` の Alpha Vantage TIME_SERIES_DAILY が、

- NYSE venue-specific listing price
- US consolidated / composite aggregation
- provider 独自 aggregation

のいずれかを **公式契約として確定していない**。

したがって:

- OpenFIGI に `micCode=XNYS` を付けても、**AV IBM series が XNYS listing price であることは自動証明されない**
- unique venue-level FIGI や MIC があっても、**AV PRICE ↔ listing-specific join / DailyPrice は NO-GO**（Price venue semantics 解決まで）

この blocker は **OpenFIGI / MIC 層とは独立**に記録する。
...
| その FIGI が OpenFIGI listing/venue-level **external identity candidate** | **可（条件付き）**（equity venue-level FIGI の公式粒度；SecurityId ではない） |
| その FIGI が ISO MIC 標準 venue と結ばれている | **未証明**（MIC 未保持；別レイヤ） |
| その FIGI / MIC が AV PRICE series の venue semantics と一致 | **未証明**（**AV Price venue semantics High blocker**） |
| その FIGI が「正しい SecurityId」 | **不可** |
| share class が一意に分離済み | **不可**（`shareClassFIGI` を evidence モデル化していない） |
| listing validity / ticker history | **不可** |
| DailyPrice に使える | **不可**（SecurityId + currency + Price venue semantics 未解決） |

**要約:** CANDIDATE = **provider symbol ↔ FIGI binding candidate provenance**。  
**≠** Security identity resolved。  
**≠** AV PRICE listing-level join resolved。  
**≠** ISO MIC standardized venue resolved（未保持時）。

---

## 7. Listing identity — minimum evidence（レイヤ分離）

### 7.1 OpenFIGI listing external identity candidate

unique venue-level FIGI + exact OpenFIGI request/response provenance + ambiguity 0 があれば:

→ **OpenFIGI 上の listing/venue-level external identity candidate は可**

MIC が無くても、この候補自体は無効化しない。

### 7.2 Standardized venue evidence（ISO MIC）

cross-provider venue join / ISO venue code を主張する場合:

→ **実際の listing に適用される explicit MIC evidence**（Operating / Segment は ISO 10383 に従う；一律 Operating MUST ではない）

`exchCode` alone は不足し得る（MIC 非等価；複合 code / Taiwan legacy）。

### 7.3 AV PRICE ↔ listing join（独立）

Alpha Vantage TIME_SERIES_DAILY の venue semantics が未確定な限り:

→ unique FIGI や MIC があっても **listing-specific Price join / DailyPrice は不可**

| Evidence | Role |
| --- | --- |
| ProviderSymbolBindingEvidence = CANDIDATE | symbol↔FIGI request/response provenance |
| Venue-level FIGI（equity） | OpenFIGI listing external identity candidate |
| Explicit MIC（optional but distinct） | ISO standardized venue evidence |
| AV Price venue semantics contract | PRICE series がどの listing/composite を指すかの独立証明 |
...
US consolidated / composite / venue-specific のいずれかを公式に確定するまで、単一 exchange listing FIGI への **Price join 固定は誤り**になり得る。

---

## 8. Venue identity — minimum evidence

| Candidate approach | OpenFIGI listing external id candidate | Standardized venue (MIC) | AV Price listing join | SecurityId | DailyPrice |
| --- | --- | --- | --- | --- | --- |
| A. OpenFIGI `exchCode` only | 補助 | **不足**（MIC 非等価） | NO（semantics 未確定） | NO-GO | NO-GO |
| B. unique venue-level FIGI（+ provenance） | **可（候補）** | 未強化 | NO（AV semantics 未確定なら join NO） | NO-GO | NO-GO |
| C. FIGI + explicit MIC | 可 | **強化** | なお AV semantics 未確定なら join NO | class/validity 次第 | NO-GO |
| D. FIGI + MIC + shareClass + **AV venue semantics** | 可 | 強化 | **条件付き検討可** | issuance 検討候補 | なお currency 等 |
| E. Provider-specific listing id | 契約次第 | 契約次第 | 契約次第 | 契約次第 | 契約次第 |

**MIC の役割:** ISO 標準 venue evidence / cross-provider venue join。FIGI を無効化しない。  
**Operating / Segment:** 当該 listing に適用される MIC を ISO 10383 evidence に従って保持。Operating 一律 MUST ではない。  
**現行 main の最大 Price join ギャップ:** AV TIME_SERIES_DAILY venue semantics 未確定（§5.6）。
...
| Scenario | Effect |
| --- | --- |
| Same share class / multiple venues | venue-level FIGI が分かれ得る；MIC は標準 venue 補強。AV Price semantics 無しでは Price join 不可 |
...
| Case | Inputs | Verdict |
| --- | --- | --- |
| 1 | unique venue-level FIGI + exact OpenFIGI provenance | **OpenFIGI listing external identity candidate = 可**。SecurityId **NO-GO**。AV Price listing join は semantics 未確定なら **不可** |
| 2 | unique FIGI + `exchCode` | FIGI candidate は維持。`exchCode` は MIC 非等価 → standardized venue **未強化**。AV join なお **不可**（semantics） |
| 3 | unique FIGI + explicit MIC | OpenFIGI listing candidate + **standardized venue 強化**。ただし AV PRICE semantics 未解決なら listing-specific Price join / DailyPrice **NO-GO** |
| 4 | FIGI + MIC + shareClass + AV venue semantics | SecurityId **issuance 検討候補**（currency は別）。DailyPrice はなお currency 等 |
...
| 8 | same share class / multiple venues | venue-level FIGI / MIC で OpenFIGI 側一意化は可。AV Price join は series semantics 次第 |
| 9 | current mapping only / old price | **past backfill FORBIDDEN** |
| 10 | venue evidence が price より後 | eligibility は **後側**（`max`） |
| 11 | `exchCode` だけで MIC 推測 | **FORBIDDEN** |
| 12 | timezone / US symbol から venue 推測 | **FORBIDDEN** |
| 13 | OpenFIGI `micCode=XNYS` + AV `symbol=IBM` only | MIC/FIGI があっても **AV series が XNYS listing とは自動証明されない** → Price join **NO-GO** |
...
- venue（MIC）明示（dual-list / standardized venue join が必要なとき；一律 Operating MUST ではない）
- ambiguity = 0
...
**AV Price venue semantics は DailyPrice / listing-specific Price join の独立 blocker**（SecurityId identity とは分離）。

---

## 14. DailyPrice boundary（文書のみ）

OpenFIGI listing external identity candidate / MIC 強化 ≠ DailyPrice GO。

DailyPrice 追加 MUST（既存型と一致）:

- resolved `SecurityId`
- explicit **trading** currency
- **AV PRICE venue semantics** が listing / composite と整合すること（公式契約）
...
| OpenFIGI listing external identity candidate | unique venue-level FIGI + provenance | **PARTIAL / 可（候補）** |
| Standardized venue (ISO MIC) | `micCode` filter は公式対応；現行未使用。`exchCode` alone 不足 | **PARTIAL / FAIL（現状）** |
| AV PRICE ↔ listing join | OpenFIGI では AV series semantics を証明できない | **FAIL**（独立 High blocker） |
...
**結論:** OpenFIGI は listing/venue の **重要な一次 source**であり、unique venue-level FIGI は **listing external identity candidate** になり得る。  
ただし **単独で SecurityId / AV Price listing join / DailyPrice / Real Backtest を開かない**。  
AV Price venue semantics・share-class / validity・（必要時）MIC は別レイヤ。
...
| --- | --- | --- |
| **AV TIME_SERIES_DAILY venue semantics**（最大 Price join ギャップ） | **公式 Alpha Vantage 資料のみ**（docs / support 等）。client 実装禁止 | **No**（次 Gate = 文書調査） |
| ISO MIC as first-class venue id | OpenFIGI `micCode` filter + ISO 10383 official MIC list（SWIFT RA） | No（AV semantics の後） |
...
| Status | When |
| --- | --- |
| `OPENFIGI_LISTING_CANDIDATE` | CANDIDATE binding + unique venue-level FIGI + ambiguity 0（MIC 無しでも可） |
| `STANDARDIZED_VENUE_EVIDENCE` | explicit MIC（Operating/Segment は ISO 10383 に従う） |
| `PRICE_VENUE_SEMANTICS_UNRESOLVED` | AV TIME_SERIES_DAILY の listing/composite 契約未確定 |
| `AMBIGUOUS_VENUE` | multi venue / multi FIGI / conflicting MIC |
| `MISSING_STANDARDIZED_VENUE` | ISO MIC 無し（`exchCode` only 含む；FIGI 候補は別） |
...
`LISTING_CANDIDATE` を MIC 必須と定義しない（FIGI / MIC 責務分離）。
...
| **AV Price venue semantics** | （未独立化） | **High（独立 blocker）** | `symbol=IBM` 等が venue-specific / consolidated / composite / provider agg のいずれかか未確定。MIC/FIGI があっても Price join を開かない |
| Venue / MIC（standardized） | High | **High（維持）** | `exchCode≠MIC`；現行未保持。FIGI を無効化しない。Forward Research は CONDITIONAL 維持のため Critical へ上げない |
...
| # | Question | Answer |
| --- | --- | --- |
| A | unique venue-level FIGI + exact provenance だけで何が言えるか | **OpenFIGI listing external identity candidate = 可**。SecurityId resolved / AV Price listing join / DailyPrice は **不可** |
| B | OpenFIGI `exchCode` だけで venue は確定可能か | **いいえ**（MIC 非等価；複合 code / Taiwan legacy） |
| C | MIC は必要か | **standardized venue / cross-provider venue join を主張するなら必要になり得る**。FIGI 候補の成立条件ではない。Operating 一律 MUST ではない |
| D | Operating / Segment のどちらが必要か | **実際の listing に適用される explicit MIC**。Segment が効く場合は Segment MIC と corresponding Operating MIC の関係を ISO 10383 evidence に従い保持。推測禁止 |
...
| G | OpenFIGI 単独で十分か | **listing external identity candidate は PARTIAL 可；SecurityId / AV Price join / DailyPrice / Backtest は不十分** |
| H | 次の最小作業 | **Alpha Vantage Price Venue Semantics Gate**（下記 §20）。MIC mapping evidence PoC は後続 |

---

## 20. Next minimal work（1つ）

### 選択: **Alpha Vantage Price Venue Semantics Gate**（文書調査のみ）

**MIC mapping evidence PoC ではない。** AV Price venue semantics が現行の最大 Price join ギャップであるため先に実施する。

調査対象（**公式 Alpha Vantage 資料のみ**を一次根拠）:

- `TIME_SERIES_DAILY` の `symbol` semantics
- US ticker の exchange specificity
- consolidated / composite の有無
- exchange-qualified symbol の公式仕様
- `SYMBOL_SEARCH` 等から exchange / region 情報が得られるか
- PRICE series と specific MIC / listing を安全に結べるか
...
| Lens | Note |
| --- | --- |
| Technical | MIC / richer master / AV semantics 契約が必要でも技術要件として記録 |
| Economic | 有料 exchange/security master は口座 1 万円では **ECONOMICALLY_UNSUITABLE** になり得る |
| Rule | 「高いから技術 FAIL」としない。OpenFIGI `micCode` と AV 公式 docs は無料範囲で調査できる点が有利 |
...
| Role | Verdict | Note |
| --- | --- | --- |
| SE | **PASS (review)** | FIGI/MIC 分離・AV semantics blocker・次最小作業が明確 |
| Programmer | **PASS (no code)** | Kotlin 変更なし |
| Data Integrity | **PASS** | CANDIDATE≠Security；FIGI≠MIC；AV Price venue semantics 独立 High；backfill 禁止維持 |
