...
| Ticker Events / change effective date / knownAt 未確立 | **High**（live schema PASS；knownAt 転用は引き続き禁止） |
...
| D. Live Ticker Events schema probe / live archive validation | **DONE**（`LIVE_SCHEMA_VERIFIED`） |
