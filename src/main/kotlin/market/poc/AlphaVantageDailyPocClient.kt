...
/**
 * Alpha Vantage TIME_SERIES_DAILY の最小 live/local client（PoC）。
 *
 * - SecurityId へ割当しない
 * - historical knownAt を生成しない
 * - mock fallback しない
 * - HTTP 200 + Error/Information/Note envelope は Fail-Closed
 * - fetchedAt は body 受信・parse・symbol 一致後のみ打刻
 */
class AlphaVantageDailyPocClient(
    ...
    fun fetchDailySeries(symbol: String): AlphaVantageDailyFetchResult {
        ...
        if (status !in 200..299) {
            throw AlphaVantagePocException("Unexpected Alpha Vantage HTTP $status for symbol=$symbol")
        }
        if (bodyBytes.isEmpty()) {
            throw AlphaVantagePocException("Empty Alpha Vantage body for symbol=$symbol")
        }

        val rawJson = String(bodyBytes, StandardCharsets.UTF_8)
        val sha = AlphaVantageDailyParser.sha256Hex(bodyBytes)
        val series = AlphaVantageDailyParser.parse(rawJson, requestedSymbol = symbol)
        val fetchedAt = clock()
        val redactedEndpoint = endpoint.replace(encodedKey, "<redacted>")

        return AlphaVantageDailyFetchResult(
            evidence =
                AlphaVantageFetchEvidence(
    ...
    companion object {
        ...
        fun fromEnvironment(): AlphaVantageDailyPocClient {
            val envKey = System.getenv(ENV_API_KEY)?.trim().orEmpty()
            return if (envKey.isNotEmpty()) {
                AlphaVantageDailyPocClient(apiKey = envKey, apiKeySource = ApiKeySource.ENVIRONMENT)
            } else {
                AlphaVantageDailyPocClient(apiKey = DEMO_API_KEY, apiKeySource = ApiKeySource.DEMO)
            }
        }
    }
}
